...
<verification>
[Overall phase checks. Include Playwright coverage if user-visible UI changed.]
</verification>
...
**Frontend rule:** If the task changes user-visible UI, the verify field must include a Playwright test that exercises the change. If Playwright is not configured, add a setup task first.
</field>
...
See `./checkpoints-core.md` for checkpoint guidance. Use `./checkpoints.md` for examples.
</task_types>
...
- [ ] Playwright tests pass for any user-visible UI changes
</verification>
```
