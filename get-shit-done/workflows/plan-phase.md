...
4. ~/.claude/get-shit-done/references/checkpoints-core.md
...
**Conditional:** If the phase includes deployment, billing, databases, or third-party integrations, load `~/.claude/get-shit-done/references/cli-automation.md` for detailed provider commands.
...
- **UI rule**: If task changes user-visible UI, include a Playwright test in Verify and add it to plan-level Verification. If Playwright is not configured, add a setup task first.
...
See ~/.claude/get-shit-done/references/checkpoints-core.md for checkpoint structure. Use ~/.claude/get-shit-done/references/checkpoints.md for examples.
</step>
...
<verification>
[Overall plan verification checks. Include Playwright tests for user-visible UI changes.]
</verification>
