...
2. For each task:
   ...
   **If `type="auto"`:**
   ...
   - If the task changes user-visible UI, run the Playwright test(s) specified in `<verify>`. If Playwright is not set up or tests are missing, add coverage before marking done.
...
See ~/.claude/get-shit-done/references/checkpoints-core.md for checkpoint guidance. Use ~/.claude/get-shit-done/references/checkpoints.md for examples.
</step>
